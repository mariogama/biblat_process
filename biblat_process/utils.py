...
class Configuration(SingletonMixin):
    ...
    def from_env(cls):
        try:
            filepath = os.environ['BIBLATP_SETTINGS_FILE']
        except KeyError:
            logger.warning('missing env variable BIBLATP_SETTINGS_FILE, no presets available')
            return {}
    ...
    def from_file(cls, filepath):
        ...
        try:
            ...
            logger.warning('file defined on BIBLATP_SETTINGS_FILE environment variable not found (%s), no presets available', filepath)
            return {}
